...
**With the elbow rule, we should use 4 PCs.**
...
```{r}
# apply kmeans on 4 centers/clusters
pca_ret_unscaled_kmeans <- kmeans(pca_ret_unscaled$x, centers = 4)

# color indicates the true cancer type
# shape indicates the cluster results
p3 <- data.table(x = pca_ret_unscaled$x[,1], 
                 y = pca_ret_unscaled$x[,2], 
                 col = as.factor(brca_subtype), 
                 cl = as.factor(pca_ret_unscaled_kmeans$cluster), 
                 centers_x = pca_ret_unscaled_kmeans$centers[,1], 
                 centers_y = pca_ret_unscaled_kmeans$centers[,2])  %>%
  ggplot() +
...
```{r}
p4 <- data.frame(x = pca_ret_unscaled$x[,1],
                 y = pca_ret_unscaled$x[,2],
                 col = as.factor(pca_ret_unscaled_kmeans$cluster),
                 cl = as.factor(pca_ret_unscaled$cluster)) %>%
  ggplot() +
  geom_point(aes(x = x, y = y, col = col, shape = cl)) +
...
# centered
x_ret_unscaled <- prcomp(x_patient_sub, center = T, scale. = F)
