...
IQ.val <- IQ[validation.index, ] # extract validation data from IQ
IQ.tmp <- IQ[-validation.index, ] # remove validation data from IQ, store as temporary data from which testing and training data will be extracted
n1 <- nrow(IQ.tmp)
IQ.test <- IQ[sample(n1, n*0.20), ]
IQ.train <- IQ[-sample(n1, n*0.20), ]
```
...
```{r fit1 summmary, echo=F}
fit1
data.table::data.table(fit1$frame)
```
    
**There are four end nodes, meaning prediction for LogIncome only takes one of the four values. For example, if Gender is male and years of Education completed before 2006 is less than 15.5, then the predicted LogIncome is 11.16.**
...
**Michelle's LogIncome is predicted to be 9.96.**
...
```{r fit2 summary, echo=F}
fit2
data.table::data.table(fit2$frame)
```
...
test.error.fit2
```
**Since fit2 uses all of the predictor variables, we would expect its training error to be always less than that from fit1 because fit2 should provide a better fit to the training data. However, we cannot expect the testing error of fit2 to be always less than that from fit1 because using more predictor variables doesn't always result in a better model, and it is possble that fit2 overfits the training data, which can result in a higher testing error.**
...
test.error.bag <- mean((predict(fit.bag.2.predict, IQ.test) - IQ.test$LogIncome)^2)
```
