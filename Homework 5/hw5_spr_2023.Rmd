...
**The two leading words and their coefficients are refreshing (2.33) and perfection (1.82). The coefficients indicate that when a review contains the word refreshing or perfection, the review has a 2,334% or 1,819% increase, respectively, in the odds of being a positive versus a negative review.**
```{r positive words}
...
**Our accuracy for this model is 87.4%, which equates to a misclassification error of 12.56%. The loss for this model is 0.31.**
...
```{r use RF to build predictive model based on PCs}
...
```{r testing error of RF based on pca}
...
em.test.err
```
**The testing error of the ensemble model is 0.1228.**
