...
test.error.fit2.t <- mean((predict(fit2.t, IQ.test) - IQ.test$LogIncome)^2)
test.error.fit2.p <- mean((predict(fit2.p, IQ.test) - IQ.test$LogIncome)^2)
test.error.fit2.t 
test.error.fit2.p
```
...
fit.bag.2.predict.c <- rep(fit.bag.2.predict, length(IQ.test$LogIncome))
mean((fit.bag.2.predict.c - IQ.test$LogIncome)^2)
```
...
# number of trees
fit.rf <- randomForest(LogIncome~., IQ.train, mtry=9, ntree=250) # after changing ntree
...
# mtry = 9
fit4 <- randomForest(LogIncome~., IQ.train, mtry=9, ntree=250)
plot(fit4)
fit4$mse[250] 
```
...
**The predicted LogIncome value for Michelle is 9.736415.**
...
```{r fit5}
fit.single <- randomForest(LogIncome~., IQ.train, mtry=30, ntree=1)
fit.bagging <- randomForest(LogIncome~., IQ.train, mtry=30, ntree=250)
fit.rf <- randomForest(LogIncome~., IQ.train, mtry=9, ntree=250)
c(fit.single$mse, fit.bagging$mse[250], fit.rf$mse[250])
```
