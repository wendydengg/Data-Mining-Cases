...
ASVAB_PC1 <- pc.tests.loading %>% select(PC1) %>% arrange(-PC1)
ASVAB_PC2 <- pc.tests.loading %>% select(PC2) %>% arrange(-PC2)
ASVAB_PC1
ASVAB_PC2
```
...
```{r PC1 PC2 Income}
model <- lm(logIncome ~ ASVAB_PC1$PC1 + ASVAB_PC2$PC2 + Gender, data = IQ)
summary(model)
