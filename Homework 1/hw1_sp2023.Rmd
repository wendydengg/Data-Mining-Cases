...
Some rows were completely void of data because the worktime is less than or equal to 5 seconds, meaning the people who took the survey probably did not look through anything and clicked submit. In that case, those rows can be disregarded. Since we are interested in the number of Wharton listeners and Sirius listeners, rows with missing entries in either "wharton" or "sirius" can also be disregarded. As for other missing entries, since we are interested in finding the type of audience for both radio channels, we would want all entries to be filled to avoid inconsistency in our data analysis, so we are left with rows with all entries filled in.
...
cleaned_results %>%
  filter(gender == 'Male') %>%
  summarise(ave_age = sum(age)/1007)
```
